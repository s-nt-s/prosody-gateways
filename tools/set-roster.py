...
class RosterExporter(ClientXMPP):
    ...
    async def __export(self, _event: Any) -> dict[str, MyUser]:
        ...
        for jid, item in roster_items.items():
            ...
            user = mk_user(jid.jid, item, user_group)

            contacts[jid.jid] = user

        for jid in FIX.keys():
            if "@" not in jid or not jid.endswith(TAIL_XMPP_DOMAIN):
                continue
            if jid not in contacts:
                user = mk_user(jid, None, user_group)
                if user:
                    contacts[jid] = user
...
def mk_user(jid: str, item: dict | None, user_group: dict[str, tuple[str]]) -> MyUser:
    if item is None:
        item = {"name": None, "groups": None}
    user = FIX.get(jid, MyUser(
        name=item['name'],
        #groups=to_tp(*item["groups"])
    ))
    group_by_room = user_group.get(jid)

    if jid.endswith(GG) and "Deleted User" == user.name:
        user = user._replace(
            groups=["🗑", ]
        )
    if group_by_room:
